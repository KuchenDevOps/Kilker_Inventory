...
> `drizzle-kit` (migraciones en `kilker-inventario/server/db/migrations/`, `0000`–`0025`).
> Idioma: español. Última actualización: 2026-08-11.
...
| Tabla | Contenido |
|-------|-----------|
| `stock_movements` | **Libro append-only**: el trigger (migraciones `0001` y `0039`) rechaza todo DELETE y solo admite corregir `unit_value`, `total_value`, `supplier_invoice_number` y `supplier_invoice_date` de un movimiento `entrada` (ver `stock_movement_edits`). `type`, `quantity` **con signo** (+ entra / − sale), `unit_value`, `total_value`, `invoice_id`, `transfer_id`, `reverses_movement_id` (liga la reversa al original), `supplier_invoice_number`/`_date` (factura del proveedor en entradas) y `"Folio"` (folio interno de entrada, único por tienda). **`iva` y `total_to_pay` son columnas GENERADAS** (`round(total_value*0.16,2)` y `round(total_value,2) + iva`), mismo patrón que `invoices` y `expenses`, pero **sólo para `type='entrada'`** (en el resto del kardex valen 0: ahí `total_value` es negativo y un IVA negativo sería dinero inventado). `total_value` es el **costo limpio** —la compra del negocio, lo único que ve el FIFO y la valuación— y `total_to_pay` es **lo que se le paga al proveedor**: contra eso se miden `entry_payments`, el saldo y el estado de pago. |
...
| Tabla | Contenido |
|-------|-----------|
| `invoices` | Comprobante interno (sin CFDI/SAT). `folio` (único por tienda, `<CODE>-0001`), `store_id`, `customer_id`, `created_by`, `status`, `payment_method`, `channel`, `discount_pct`/`discount_amount`, `total_amount`, `issued_at` (admite fecha retroactiva) y `voided_at/by/reason`. **`iva` y `total_to_pay` son columnas GENERADAS** (`round(total_amount*0.16,2)` y `total_amount + iva`), mismo patrón que `expenses`: `total_amount` es el **subtotal** (el ingreso) y `total_to_pay` es **lo que se le cobra al cliente** — contra eso se miden abonos, saldo y el corte de caja. |
...
| `invoice_items` | Líneas. `quantity`, `unit_price` (**snapshot** al vender), `line_total`. `discount_type`/`discount_value`/`tax_rate` existen pero hoy no se llenan (el IVA es de la FACTURA, no de la línea: vive en `invoices.iva`). **Venta por kit:** `kit_id` + `kit_sku`/`kit_name` (**snapshot**) + `kit_quantity` marcan de qué kit salió la línea; null = producto suelto. **Entrega como muestra:** `sample_product_id` + `sample_sku`/`sample_name` (**snapshot**); `product_id` sigue siendo SIEMPRE el producto **base** (el que movió inventario), así que ningún reporte cambia. |
