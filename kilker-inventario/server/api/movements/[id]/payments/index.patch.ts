// ───────────────────────────────────────────────
//  PATCH /api/movements/:id/payments — asignar cuenta a todos los pagos (admin)
// ───────────────────────────────────────────────
...
export default defineEventHandler(async (event) => {
  ...
  const doc = await db.query.stockMovements.findFirst({
    where: eq(stockMovements.id, id),
    columns: { id: true }
  })
  if (!doc) throw createError({ statusCode: 404, statusMessage: 'Entrada no existe' })
